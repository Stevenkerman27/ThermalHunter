...
表格模型、DQN 权重和传感器统计均为必需输入，缺失时立即报错。结果是最终高度减去初始高度，逐场景 CSV 默认写入 `trainresult/evaluation_episodes.csv`，比较图默认写入 `trainresult/compare_eval_result.png`。`--w-accel-weight` 必须与所评估的稳态模型奖励配置一致。
